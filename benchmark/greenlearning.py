...
if __name__ == '__main__':
    ...
    pbar = trange(epochs)
    for ep in pbar:
        pbar.set_description("train l2 {:.2e} - test l2 {:.2e} - KernelNorm {:.2e}".format(train_rl2, test_rl2_best, test_matrl2_best))
        glnet.train()
        train_rl2 = 0
        ...
        for u, w in train_loader:
            ...
            rnd_rows = torch.randint(low=0, high=2**13+1, size=(8,)).to(device)
            w_rows = w[:,:,rnd_rows]
        ...
        test_matrl2 = matrl2_error(Khh, Khh_)
        ...
        if test_rl2 < test_rl2_best:
            ...
            torch.save(glnet, model_outpath)
